...
import { z } from 'zod';
import { ProdutoMarcaArgs } from '../types/ProdutoMarcaEntity';
...
import { EDT_MARCA } from '@/infra/graphql/mutations/DProduto/Produto/mutation_edt_narca';
...
type FormProps = z.infer<typeof ProdutoMarcaArgs>;
type Tprops = {
    create?: crudForm,
...
export function EDTProdutoMarcaForm(props: Tprops) {

    let errorGql: TTopoFormErros = { erro: false, msg: '' }


    const [ExecMutation, { data, loading, error }] = useMutation(EDT_MARCA, {
    ...
    const _form = useForm<FormProps>({
        resolver: zodResolver(ProdutoMarcaArgs),
        defaultValues: {
    ...
    const _onSubmit: SubmitHandler<FormProps> = (data: FormProps) => {
        ...
        ExecMutation({ variables: { marcaDeProdutoEdicaoId: props.dataForm?._id, dados: { ...dto } } })
    }
