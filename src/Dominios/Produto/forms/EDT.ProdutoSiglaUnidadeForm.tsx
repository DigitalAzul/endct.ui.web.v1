...
import { EDT_SIGLA_UNIDADE_PRODUTO } from '@/infra/graphql/mutations/DProduto/Produto/mutation_edt_sigla_unidade';
...
import { ProdutoUniMedSiglaEditar } from '../types/ProdutoUniMedSiglaEntity';


type FormProps = z.infer<typeof ProdutoUniMedSiglaEditar>;
type Tprops = {
    create?: crudForm,
...
export function EDTProdutoSiglaUnidadeForm(props: Tprops) {

    let errorGql: TTopoFormErros = { erro: false, msg: '' }


    const [ExecMutation, { data, loading, error }] = useMutation(EDT_SIGLA_UNIDADE_PRODUTO, {
    ...
    const _form = useForm<FormProps>({
        resolver: zodResolver(ProdutoUniMedSiglaEditar),
        defaultValues: {
    ...
    const _onSubmit: SubmitHandler<FormProps> = (data: FormProps) => {
        ...
        ExecMutation({ variables: { siglaDeUnidadeDeMedidaDeProdutoEdicaoId: props.dataForm?._id, dados: { ...dto } } })
